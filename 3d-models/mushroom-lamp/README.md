...
2. **`base.stl` (받침대)** — 돌 모양 받침대, 작은 버섯 장식과 기둥을 감싸듯 8군데에
   ...
   무관)로 출력하면 됩니다. 고사리 잎은 얇은 블레이드 형태(두께 1.6mm)라 출력 시
   최소 벽 두께를 지원하는 프린터/소재인지 업체와 확인해 주세요 (너무 얇으면 부러지기
   쉬워 레진 출력을 추천).
...
- 버섯 2개 (큰 것 1개 + 작은 장식 1개), 기둥 둘레 8다발의 고사리로 구성 — 이끼 질감,
  달팽이 모양 소용돌이 장식, 반딧불 핀(구슬 막대)은 생략
