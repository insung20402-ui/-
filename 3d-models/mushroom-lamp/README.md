...
2. **`base.stl` (받침대)** — 돌 모양 받침대, 작은 버섯 장식과 기둥을 감싸듯 8군데에
   배치된 고사리(fern) 다발이 붙어 있고, 윗면에 쉐이드 기둥이 꽂히는 보스(소켓)가
   있습니다. 아래쪽에서 접근하는 내부 공간에 LED 드라이버/배터리/USB 모듈을 넣고,
   별도의 바닥 커버(원판)로 마감하는 구조입니다. **불투명 소재**(PLA, 레진 등 색상
   무관)로 출력하면 됩니다.
   고사리는 보내주신 사진처럼 중심 줄기(rachis)에서 작은 잎(leaflet)들이 좌우로
   갈라져 나오는 실제 깃털형 잎맥 구조로 만들었습니다. 잎맥이 두께 1.2~1.6mm의
   가느다란 구조라 출력 시 최소 벽 두께를 지원하는 프린터/소재인지 업체와 확인해
   주세요 (FDM보다 디테일이 잘 살아나는 **레진(SLA/DLP) 출력을 강력 추천**합니다).
...
- 버섯 2개 (큰 것 1개 + 작은 장식 1개), 기둥 둘레 8다발의 깃털형 고사리로 구성 —
  이끼 질감, 달팽이 모양 소용돌이 장식, 반딧불 핀(구슬 막대)은 생략
