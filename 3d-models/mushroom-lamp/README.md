...
2. **`base.stl` (받침대)** — 돌 모양 받침대, 작은 버섯 장식이 붙어 있고, 윗면에 쉐이드
   기둥이 꽂히는 보스(소켓)가 있습니다. 아래쪽에서 접근하는 내부 공간에 LED 드라이버/
   배터리/USB 모듈을 넣고, 별도의 바닥 커버(원판)로 마감하는 구조입니다.
   **불투명 소재**(PLA, 레진 등 색상 무관)로 출력하면 됩니다.
...
- 버섯 2개 (큰 것 1개 + 작은 장식 1개)로 축소, 고사리/이끼는 생략
