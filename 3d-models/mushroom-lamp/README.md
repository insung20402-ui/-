...
| 항목 | 값 |
|---|---|
| 전체 높이 (받침대 포함) | 약 204 mm |
| 갓(캡) 지름 | 190 mm |
| 받침대 지름 | 약 210 mm (불규칙한 바위 윤곽, 완전한 원이 아님) |
...
2. **`base.stl` (받침대)** — 완전한 원이 아니라 **불규칙하게 깨진 층상(sedimentary)
   바위 윤곽**으로 만들었습니다 (보내주신 언더사이드 참고 사진처럼, 가장자리가 층층이
   울퉁불퉁하게 깨진 모양). 중심/윗면(버섯·이끼가 자리한 곳)은 매끈하고, 바깥 가장자리로
   갈수록 절벽처럼 층진 굴곡이 강해집니다. 그 위에 아래 장식들이 실제 표면 높이에
   맞춰(레이캐스팅으로 계산) 자연스럽게 자리잡고 있습니다. 아래쪽에서 접근하는 내부
   공간에 LED 드라이버/배터리/USB 모듈을 넣고, 별도의 바닥 커버(원판)로 마감하는
   구조입니다. **불투명 소재**(PLA, 레진 등 색상 무관)로 출력하면 됩니다.
   - 기둥을 감싸듯 8군데에 배치된 고사리(fern) 다발 — 중심 줄기(rachis)에서 작은
     잎(leaflet)들이 좌우로 갈라져 나오는 실제 깃털형 잎맥 구조
   - 작은 버섯 장식 1개 + 이끼 사이로 고개를 내민 미니 버섯 새싹 2개
   - 달팽이 껍질처럼 말려 올라가는 소용돌이 덩굴(curl) 4개
   - 반딧불처럼 구슬이 달린 얇은 핀(light-stick) 3개
   - 빈 땅을 채우는 자갈/이끼 범프 텍스처 150개
   잎맥·덩굴·핀은 두께 1.2~2mm의 가느다란 구조라 출력 시 최소 벽 두께를 지원하는
   프린터/소재인지 업체와 확인해 주세요 (FDM보다 디테일이 잘 살아나는 **레진(SLA/DLP)
   출력을 강력 추천**합니다).
...
- 버섯은 큰 것 1개 + 작은 장식 1개 + 미니 새싹 2개로 축소 (원본은 버섯 군집이 더 큼)
- 갓 표면의 반점 무늬, 기둥의 소용돌이 조각 무늬는 생략 (원한다면 도색으로 표현하거나,
  추가 요청 시 형상에 엠보싱 패턴을 넣는 것도 가능합니다)
